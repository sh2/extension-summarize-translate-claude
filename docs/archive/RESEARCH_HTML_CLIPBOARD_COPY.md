...
- 由来 Issue: <https://github.com/sh2/extension-summarize-translate-gemini/issues/50>
- 調査日: 2026-09-14
- 状態: 調査のみ（実装未着手）
...
| 箇所 | 内容 |
| --- | --- |
| `extension/popup.js` `copyContent` | `navigator.clipboard.writeText()` のみ |
| `extension/results.js` `copyContent` | 同上。`responseContent` と会話のテキストを `\n\n` で連結 |
| `extension/utils.js` `convertMarkdownToHtml`（135行付近） | 表示用 HTML は marked + DOMPurify で生成済み |
| `extension/utils.js` `exportTextToFile`（166行付近） | `.txt` / `text/plain` 固定 |
...
Claude版は `canWriteHtml` 判定と `try/catch` によるプレーンテキスト
フォールバックを実装して対応する。
...
15 ロケール分の i18n キー追加が必要になる。コピー側を「HTML 併記」方式にすれば
コピー機能の i18n 変更は不要。

## 7. 実装する場合の変更箇所

- `extension/utils.js` の `UI helpers` に共通ヘルパーを追加（Clipboard API は
  副作用を伴うため `Pure utilities` には置かない）。
- `extension/popup.js` / `extension/results.js` の `copyContent` から呼び出す。
- HTML は `#content`（results では `#conversation` も）を使えば表示と
  一致し、CJK emphasis 修正済みの内容をそのまま利用できる。
- `exportTextToFile` を拡張子・MIME 指定可能に拡張（`.md` / `.html` 対応）。
- テストは `test/` にクリップボードのモックが未整備なため、
  `navigator.clipboard.write` のスタブを追加する（Claude版はテスト基盤が無いため
  手動検証で代替する。計画書 §4.2 を参照）。
...
- 添付画像（conversation の image parts）は現状テキストのみコピー対象。HTML に
  含めると data URL 化で肥大するため、除外が妥当。Claude版は添付画像を
  表示しないため対象は無いが、Markdown 出力に data URL 画像が含まれた場合の
  保険として `img[src^="data:"]` を除去する。
